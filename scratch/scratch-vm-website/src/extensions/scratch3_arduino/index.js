...
*/
        ...
        console.log("Arduino constructor done!");
    ...
    scan () {
        ...
        if (this._uno) {
            this._uno.disconnect();
        }
        this._uno = new UNO(this._runtime, 'Arduino', {
            majorDeviceClass: 8,
            minorDeviceClass: 1
    ...
    connect (id) {
        ...
        console.log(id);
        if (this._uno) {
            this._uno.connectPeripheral(id);
        }
    }
    ...
    isConnected () {
        ...
        var connected = false;
        if (this._uno) {
            connected = this._uno.isConnected();
        }
    ...
    /**
     * When the Arduino peripheral connects, start polling for sensor and motor values.
     * @private
     */
    _onConnect () {
            ...
            blocks: [
                {
                    ...
                    text: 'Get Arduino Firmware Version'
                },
                {
                    opcode: 'blink',
                    blockType: BlockType.COMMAND,                    
                    text: 'Blink Arduino LED'
                },
            ],
    ...
    getFirmware() {
        console.log('Entering getFirmware...');
        this.send(new Uint8Array([0xF0, 0x6B, 0xF7]));
    }

    blink() {
        this.send(new Uint8Array([0x03]));
    }
