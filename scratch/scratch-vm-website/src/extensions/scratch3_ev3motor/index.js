// create by scratch3-extension generator
const ArgumentType = require('../../extension-support/argument-type');
const BlockType = require('../../extension-support/block-type');
const Base64Util = require('../../util/base64-util');
const {loadCostume} = require('../../import/load-costume.js');
const Cast = require('../../util/cast');
const log = require('../../util/log');
const formatMessage = require('format-message');
const SvgRenderer = require('scratch-svg-renderer');


const menuIconURI = 'data:image/svg+xml;base64,';
const blockIconURI = menuIconURI;


const EV3Motor_Name = {
    'en': 'EV3 Motor',
    'zh-cn': '乐高马达',
}

const EV3Motor_RunForever = {
    'en': 'Run Forever with Power [POWER] on Port [PORT]',
    'zh-cn': '一直运行 以功率 [POWER] 于端口 [PORT]',
};

const EV3Motor_RunSeconds = {
    'en': 'Run [SECONDS] Seconds with Power [POWER] on Port [PORT]',
    'zh-cn': '运行 [SECONDS] 秒 以功率 [POWER] 于端口 [PORT]',
};

const EV3Motor_RunCircles = {
    'en': 'Run [CIRCLES] Circles with Power [POWER] on Port [PORT]',
    'zh-cn': '运行 [CIRCLES] 圈 以功率 [POWER] 于端口 [PORT]',
};

const EV3Motor_RunDegrees = {
    'en': 'Run [DEGREES] Degrees with Power [POWER] on Port [PORT]',
    'zh-cn': '运行 [DEGREES] 度 以速率 [POWER] 于端口 [PORT]',
};

const EV3Motor_StopIt = {
    'en': 'Stop It on Port [PORT]',
    'zh-cn': '停下来 端口 [PORT]',
};


class EV3Motor {
    constructor (runtime) {
        /**
         * The runtime instantiating this block package.
         * @type {Runtime}
         */
        this.runtime = runtime;
        this.camera_status = false;
        this.locale = this._setLocale();
    }


    _setLocale () {
        let now_locale = '';
        switch (formatMessage.setup().locale){
        case 'en':
            now_locale='en';
            break;
        case 'zh-cn':
            now_locale= 'zh-cn';
            break;
        default:
            now_locale='en';
            break;
        }
        return now_locale;
    }


    getInfo() {
        return {
            id: 'ev3motor',
            name: EV3Motor_Name[this.locale],
            menuIconURI: menuIconURI,
            blockIconURI: blockIconURI,
            blocks: [
                {
                    opcode: 'runforever',
                    blockType: BlockType.COMMAND,
                    text: EV3Motor_RunForever[this.locale],
                    arguments: {
                        POWER: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 100
                        },
                        PORT: {
                            type: ArgumentType.STRING,
                            defaultValue: 'A'
                        }
                    },
                },
                {
                    opcode: 'runseconds',
                    blockType: BlockType.COMMAND,
                    text: EV3Motor_RunSeconds[this.locale],
                    arguments: {
                        SECONDS: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1
                        },
                        POWER: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 100
                        },
                        PORT: {
                            type: ArgumentType.STRING,
                            defaultValue: 'A'
                        }
                    },
                },
                {
                    opcode: 'runcircles',
                    blockType: BlockType.COMMAND,
                    text: EV3Motor_RunCircles[this.locale],
                    arguments: {
                        CIRCLES: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1
                        },
                        POWER: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 100
                        },
                        PORT: {
                            type: ArgumentType.STRING,
                            defaultValue: 'A'
                        }
                    },
                },
                {
                    opcode: 'rundegrees',
                    blockType: BlockType.COMMAND,
                    text: EV3Motor_RunDegrees[this.locale],
                    arguments: {
                        DEGREES: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 90
                        },
                        POWER: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 100
                        },
                        PORT: {
                            type: ArgumentType.STRING,
                            defaultValue: 'A'
                        }
                    },
                },
                {
                    opcode: 'stopit',
                    blockType: BlockType.COMMAND,
                    text: EV3Motor_StopIt[this.locale],
                    arguments: {
                        PORT: {
                            type: ArgumentType.STRING,
                            defaultValue: 'A'
                        }
                    },
                },
            ],
            menus: {
            }
        }
    }

    /**
     * Create data for a menu in scratch-blocks format, consisting of an array
     * of objects with text and value properties. The text is a translated
     * string, and the value is one-indexed.
     * @param {object[]} info - An array of info objects each having a name
     *   property.
     * @return {array} - An array of objects with text and value properties.
     * @private
     */
    _buildMenu (info) {
        return info.map((entry, index) => {
            const obj = {};
            obj.text = entry.name;
            obj.value = entry.value || String(index + 1);
            return obj;
        });
    }

    runforever(args, util) {
    }

    runseconds(args, util) {
    }

    runcircles(args, util) {
    }

    rundegrees(args, util) {
    }

    stopit(args, util) {
    }

}

module.exports = EV3Motor;
